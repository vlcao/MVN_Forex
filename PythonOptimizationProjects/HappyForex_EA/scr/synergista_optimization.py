...
RUN_FREQUENCY_11_YEAR = '11'

# default RUN_FREQUENCY_BY_YEAR = 1 with Symbol = EURUSD 
if (len(sys.argv[1:]) == DEFAULT_NUMBER_INT):
    RUN_FREQUENCY_BY_YEAR = DEFAULT_SECOND_NUMBER_INT
    YEAR_OUTPUT = '1y'
    SYMBOL = 'EURUSD'
    QUOTE_CURRENCY = 'USD'
    BASE_CURRENCY = 'EUR'
# retrieve SYMBOL and RUN_FREQUENCY_BY_YEAR from input text
else:
    default_len_symbol = 6
    default_len_currency = 3

    # 1-retrieve SYMBOL
    SYMBOL = str(sys.argv[1:][DEFAULT_NUMBER_INT])
    if (len(SYMBOL) == default_len_symbol):
        
        SYMBOL = SYMBOL.upper()
        # only run with desired SYMBOL
        if (SYMBOL == 'GBPUSD'
            or SYMBOL == 'EURUSD'
            or SYMBOL == 'USDJPY'):
        
            # retrieve QUOTE/BASE_CURRENCY
            QUOTE_CURRENCY = SYMBOL[:default_len_currency]
            BASE_CURRENCY = SYMBOL[-default_len_currency:]
        # default Symbol = EURUSD when input Symbol is NOT the same as desired Symbols
        else:
            SYMBOL = 'EURUSD'
            QUOTE_CURRENCY = 'USD'
            BASE_CURRENCY = 'EUR'
    # default Symbol = EURUSD when length of input Symbol is NOT correct 
    else:
        SYMBOL = 'EURUSD'
        QUOTE_CURRENCY = 'USD'
        BASE_CURRENCY = 'EUR'
        
    # 2-retrieve the RUN_FREQUENCY_BY_YEAR
    RUN_FREQUENCY_BY_YEAR = sys.argv[1:][DEFAULT_SECOND_NUMBER_INT]
    
    # only run with desired RUN_FREQUENCY_BY_YEAR
    if (str(RUN_FREQUENCY_BY_YEAR) == RUN_FREQUENCY_1_YEAR
        ...
        YEAR_OUTPUT = str(RUN_FREQUENCY_BY_YEAR) + 'y'
        
    # default RUN_FREQUENCY_BY_YEAR = 1 when input RUN_FREQUENCY_BY_YEAR is NOT the same as desired one 
    else:
        RUN_FREQUENCY_BY_YEAR = DEFAULT_SECOND_NUMBER_INT
        YEAR_OUTPUT = '1y'

print("SYMBOL = %s" % SYMBOL)
print("QUOTE_CURRENCY = %s" % QUOTE_CURRENCY) 
print("BASE_CURRENCY = %s" % BASE_CURRENCY)
print("RUN_FREQUENCY_BY_YEAR = %s" % RUN_FREQUENCY_BY_YEAR)
print("YEAR_OUTPUT = %s" % YEAR_OUTPUT)
...
# SCR_DIR_PATH = '/home/app'
FOLDER_DATA_INPUT = SCR_DIR_PATH + '/data/input/'
FOLDER_DATA_OUTPUT = SCR_DIR_PATH + '/data/output/'
FOLDER_TICK_DATA_ORIGINAL = '/' + SYMBOL + '_GAINCapital_Original'
FOLDER_TICK_DATA_MODIFIED = '/' + SYMBOL + '_GAINCapital_Modified'
...
FILENAME_POPULATION_INITIAL = 'PopulationInitial.csv'
FILENAME_POPULATION_FINAL = 'PopulationFinal.csv'
FILENAME_LOG_BACKTEST = 'synergista1_backtest.log'
FILENAME_PROFILE_BACKTEST = 'synergista1_backtest.prof'
FILENAME_LOG_GA = 'synergista1_GA.log'
FILENAME_PROFILE_GA = 'synergista1_GA.prof'
FILENAME_BEST_FITNESS = '.30.solution_optimize_last.txt'
FILENAME_BEST_FITNESS_ORDER_HISTORY = '.30.order_history_optimize_last.csv'
FILENAME_BEST_PARAMETERS = '.30.optimize_last.txt'
FILENAME_ORDER_CLOSED_HISTORY = 'OrderClosedHistory.csv'
FILENAME_ORDER_OPENED_HISTORY = 'OrderOpenedHistory.csv'
FILENAME_ORDER_DELETED_HISTORY = 'OrderDeletedHistory.csv'
FILENAME_DATE_DICT = 'DataDateDict.csv'
...
def create_multiple_tick_data_from_wholefolder_gaincapital_format(folder_name):
    ...
    folder_output_name = replace_string(folder_name, 'input', 'output')
    if os.path.exists(folder_output_name):
        shutil.rmtree(folder_output_name)
    ...
    for file_ in allFiles:
        ...
        # create an output CSV file name
        file_name = replace_string(file_, 'input', 'output')
...
# FOLDER_TICK_DATA_ORIGINAL = FOLDER_TICK_DATA_ORIGINAL + '/GBPUSD_2005_2017_GAINCapital_DateTimeCol_3/with_miliseconds/'
# MARKET_TIME_STANDARD = '1970.01.01_00:00:00.000'
# DATETIME_FORMAT = '%Y.%m.%d_%H:%M:%S.%f'
# create_multiple_tick_data_from_wholefolder_gaincapital_format(convert_backflash2forwardflash(FOLDER_DATA_INPUT + SYMBOL + FOLDER_TICK_DATA_ORIGINAL))
...
# FOLDER_TICK_DATA_ORIGINAL = FOLDER_TICK_DATA_ORIGINAL + '/GBPUSD_2005_2017_GAINCapital_DateTimeCol_2/NO_miliseconds/'
# MARKET_TIME_STANDARD = '1970.01.01_00:00:00'
# DATETIME_FORMAT = '%Y.%m.%d_%H:%M:%S'
# create_multiple_tick_data_from_wholefolder_gaincapital_format(convert_backflash2forwardflash(FOLDER_DATA_INPUT + SYMBOL + FOLDER_TICK_DATA_ORIGINAL))
#   
...
class ServerSocket(object):
    ...
    def __init__(self, parameterFilePath):
         
        try:
            self.parameterFilePath = parameterFilePath
            parameters = self.loadPropertiesFile(self.parameterFilePath)
        except Exception as e:
            raise Exception('\"%s\" doesn\'t exist, error: %s' % (
                str(parameterFilePath), str(e)))
            return
         
        try:
            self.bindIP = parameters['SERVICE_SOCKET_IP']
            self.socketPort = parameters['SERVICE_SOCKET_PORT']
            print('Server socket: %s:%s' % (str(self.bindIP), str(self.socketPort)))
             
            self.concurentListen = parameters['SERVICE_SOCKET_CONCURENT_LISTEN']
            print("Concurrent connections: %s" % (str(self.concurentListen)))
        except Exception as e:
            raise Exception('Some parameters in file \'%s\' are not provided, error: %s' % (
                str(parameterFilePath), str(e)))
            return        
         
        try:
            self.serverSocket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
            self.serverSocket.bind((self.bindIP, self.socketPort))
            self.serverSocket.listen(self.concurentListen)
#             self.serverSocket.settimeout(None)
        except Exception as e:
            raise Exception('Failed to create server socket: %s:%s, error: %s' % (
                str(self.bindIP), str(self.socketPort), str(e)))
            return
    ...
    def startReceiveData(self):
        ...
        while True:
            print("Listening from port %s" % (str(self.socketPort)))
             
            conn, addr = self.serverSocket.accept()
            if addr[0] != self.bindIP:
                print('%s:%s is trying to connect -> rejected' % (str(addr[0]), str(addr[1])))
                log.info('%s:%s is trying to connect -> rejected' % (str(addr[0]), str(addr[1])))
                conn.close()
                continue
            else:
                print('%s:%s is trying to connect -> accepted' % (str(addr[0]), str(addr[1])))
                log.info('%s:%s is trying to connect -> accepted' % (str(addr[0]), str(addr[1])))
             
            start_new_thread(self.handleRequest, (conn,))
    ...
    def handleRequest(self, conn):
        ...
        while True:
            print("waiting for new data from client ...")
            ...
            if not data:
                ...
                if data == STOP_OPTIMIZING:
                    global STOP_FLAG
                    STOP_FLAG = True
                    print('Application is being stopped by user...')
                    log.info('Application is being stopped by user...')
                    break
                 
                if data == RELOAD_PARAMETERS:
    ...
    def loadEaParameters(self, inputFilePath):
        myprops = {}
        param_start = {}
        param_step = {}
        param_stop = {}
        ...
        with open(inputFilePath, 'r') as f:
            for line in f:
                ...
                b_bracket_pos = v.find("[")
                e_bracket_pos = v.find("]")
                 
                if (b_bracket_pos >= 0 and e_bracket_pos >= 0 and b_bracket_pos < e_bracket_pos):
                    param_limit = v[b_bracket_pos + 1 : e_bracket_pos]
                    v = v[0 : b_bracket_pos]
                    v = v.strip()
                     
                    permission, start, step, stop = param_limit.split(",", 3)
                    permission = permission.strip()
                    param_start[k] = self.correctDataType(start.strip())
                    param_step[k] = self.correctDataType(step.strip())
                    param_stop[k] = self.correctDataType(stop.strip())
                     
                    if permission.upper() != "ON":
                        param_start[k] = param_stop[k] = self.correctDataType(v)
                        param_step[k] = 1
                     
                    if (param_start[k] > param_stop[k]):
                        param_start[k], param_stop[k] = param_stop[k], param_start[k]
                else:
                    param_start[k] = param_stop[k] = self.correctDataType(v)
                    param_step[k] = 1
                 
                if param_step[k] == 0: param_step[k] = 1
                param_step[k] = abs(param_step[k])
                 
                myprops[k] = self.correctDataType(v)
                log.info("%s = %s [%s, %s, %s]" % (
                    k, str(self.correctDataType(v)),
                    str(param_start[k]), str(param_step[k]), str(param_stop[k])))
        return myprops, param_start, param_step, param_stop
    ...
    def loadPropertiesFile(self, inputFilePath):
        log.info("--------------- loading setting from %s" % inputFilePath)
        myprops = {}
        with open(inputFilePath, 'r') as f:
            for line in f:
                line = line.strip()  # removes trailing whitespace and '\n' chars
                ...
                v = v.strip()
                if "#" not in v:
                    myprops[k.strip()] = self.correctDataType(v)
                    # Log.logger.debug("%s -> %s" %(k.strip(), str(LogConfig.correctDataType(v))))
                else:
                    v1, _ = v.split("#", 1)                
                    myprops[k.strip()] = self.correctDataType(v1.strip())
                    # Log.logger.debug("%s -> %s" %(k.strip(), str(LogConfig.correctDataType(v1))))
        # Log.logger.debug("--------------- FINISH LOADING PARAMETER ---------------------------")                    
        return myprops   
    ...
    def reload_parameters(self):
        ...
        app_properties = self.loadPropertiesFile("conf/app.properties")
         
        global OPT_INTERVAL_RUN_TIME
        OPT_INTERVAL_RUN_TIME = app_properties['OPT_INTERVAL_RUN_TIME']
        print('OPT_INTERVAL_RUN_TIME=%s' % (str(OPT_INTERVAL_RUN_TIME)))
        log.info('OPT_INTERVAL_RUN_TIME=%s' % (str(OPT_INTERVAL_RUN_TIME)))
         
        global SEND_RESULT_RETRY
        SEND_RESULT_RETRY = int(max(1, int(app_properties['SEND_RESULT_RETRY'])))
        print('SEND_RESULT_RETRY=%s' % (str(SEND_RESULT_RETRY)))
        log.info('SEND_RESULT_RETRY=%s' % (str(SEND_RESULT_RETRY)))
         
        global MIN_TRADE_PER_DAY
        MIN_TRADE_PER_DAY = float(app_properties['MIN_TRADE_PER_DAY'])
        print('MIN_TRADE_PER_DAY=%s' % (str(MIN_TRADE_PER_DAY)))
        log.info('MIN_TRADE_PER_DAY=%s' % (str(MIN_TRADE_PER_DAY)))
         
        global PAUSE_BETWEEN_SENDING_RESULT_RETRIES
        PAUSE_BETWEEN_SENDING_RESULT_RETRIES = int(max(0, int(app_properties['PAUSE_BETWEEN_SENDING_RESULT_RETRIES'])))
        print('PAUSE_BETWEEN_SENDING_RESULT_RETRIES=%s' % (str(PAUSE_BETWEEN_SENDING_RESULT_RETRIES)))
        log.info('PAUSE_BETWEEN_SENDING_RESULT_RETRIES=%s' % (str(PAUSE_BETWEEN_SENDING_RESULT_RETRIES)))
         
        global OPT_DAYS_BEFORE_STOP
        OPT_DAYS_BEFORE_STOP = app_properties['OPT_DAYS_BEFORE_STOP']
        print('OPT_DAYS_BEFORE_STOP=%s' % (str(OPT_DAYS_BEFORE_STOP)))
        log.info('OPT_DAYS_BEFORE_STOP=%s' % (str(OPT_DAYS_BEFORE_STOP)))
         
#         load_min_max_generation(app_properties)
         
        global SENDER_EMAIL_SERVER
        SENDER_EMAIL_SERVER = app_properties['SENDER_EMAIL_SERVER']
        print('SENDER_EMAIL_SERVER=%s' % (str(SENDER_EMAIL_SERVER)))
        log.info('SENDER_EMAIL_SERVER=%s' % (str(SENDER_EMAIL_SERVER)))
         
        global SENDER_EMAIL
        SENDER_EMAIL = app_properties['SENDER_EMAIL']
        print('SENDER_EMAIL=%s' % (str(SENDER_EMAIL)))
        log.info('SENDER_EMAIL=%s' % (str(SENDER_EMAIL)))
         
        global SENDER_PASSWORD
        SENDER_PASSWORD = app_properties['SENDER_PASSWORD']
         
        global RECEIVERS_EMAILS
        RECEIVERS_EMAILS = list()
        _ = app_properties['RECEIVERS_EMAILS']
        _ = _.split(',')
        for i in range(len(_)):
            _[i] = _[i].strip()
            if len(_[i]) > 0:
                RECEIVERS_EMAILS.append(_[i])
        print('RECEIVERS_EMAILS=%s' % (str(RECEIVERS_EMAILS)))
        log.info('RECEIVERS_EMAILS=%s' % (str(RECEIVERS_EMAILS)))
...
class HappyForexEA(object):
    ...
    def CalculateProfit_5(self, entry_price, exit_price, lots, order_type):
        ...
        profit = DEFAULT_NUMBER_FLOAT
      
        if  (order_type == OP_BUY): 
            profit = ((exit_price - entry_price) * self.Lots * (1 / self.my_point) 
                      * (PIP_VALUE_DICT[SYMBOL] * self.Lots / EXCHANGE_RATE_USD[QUOTE_CURRENCY]))
          
          
        elif (order_type == OP_SELL):
            profit = ((entry_price - exit_price) * self.Lots * (1 / self.my_point) 
                      * (PIP_VALUE_DICT[SYMBOL] * self.Lots / EXCHANGE_RATE_USD[QUOTE_CURRENCY]))
        ...
        profit = DEFAULT_NUMBER_FLOAT
        if  (order_type == OP_BUY): 
            profit = (exit_price - entry_price) * lots * ONE_LOT_VALUE - COMMISSION
          
        elif (order_type == OP_SELL):
            profit = (entry_price - exit_price) * lots * ONE_LOT_VALUE - COMMISSION
         
        # when the QUOTE_CURRENCY is NOT the USD, consider the BASE_CURRENCY
        if (QUOTE_CURRENCY != BASE_CURRENCY_USD):
             
            # when the BASE_CURRENCY is USD (Direct Quote : USDJPY, USDCHF), transfer the profit to USD value 
            # by using the rate of USD with QUOTE_CURRENCY, which is exact the exit price  
            if (BASE_CURRENCY == BASE_CURRENCY_USD):
                profit = profit / exit_price
 
            # when current pair does not include USD (Cross rate: GBPAUD, GBPJPY), transfer the profit to USD value
            # by using the rate of BASE_CURRENCY with USD (GBPAUD ==> GPBUSD rate)
            else:
                pass  
                ''' skip at this moment: 
                profit = profit * base_currency_rate with USD
                 
                '''
        # when the QUOTE_CURRENCY is the USD (Indirect Quote)
        else:
            pass                
         
        return round(float(profit), self.NDigits)
    ...
    #===============================================================================
    def MaxOrders_9(self, num, individual_ID_run_EA):
        ...
        if (ords >= num):
            ...
            for order_id in self.ORDER_OPENED_DICT.keys():
                ...
                if (order_type > 1.00):
                    ...
                    self.OrderDelete_4(individual_ID_run_EA, order_id, self.ORDER_OPENED_DICT)
...
#                     account_bal = order[BALANCE_COL_INDEX]
    ...
    #===============================================================================
    def OrderType_5(self, order_id, dict_data, individual_ID_run_EA):
        ...
        if (len(dict_data) != DEFAULT_NUMBER_INT):
            ...
            log.info("{0}: There is NO data. Data size = 0".format(individual_ID_run_EA))
                 
        return order_type
     
    #===============================================================================
    def OrderDelete_4(self, individual_ID_run_EA, order_id, dict_data):
        ''' Delete s specific order in data. '''
        ...
        if (order_id in dict_data):
            ...
            log.info("{0}: There's NO {1} in data. Data is the same as before deleting.".format(individual_ID_run_EA, order_id))
            return False
         
    #===============================================================================
    def OrderAdd_10(self, individual_ID_run_EA, order_id, order, dict_data):
        ''' Add a specific order in data. '''
     
        # add the order if it's NOT existed in the data
        if (order_id in dict_data):
            log.info("{0}: There's the oder {1} in data already.".format(individual_ID_run_EA, order_id))
            return False
        else:
    ...
    #===============================================================================
    def OrderClose_4(self, order, order_id, order_type, individual_ID_run_EA):
        ...
        * and save this deleted order in the Closed orders pools'''
        ...
        if (order_type == OP_BUY or order_type == OP_SELL):
            ...
            if (self.OrderDelete_4(individual_ID_run_EA, order_id, self.ORDER_OPENED_DICT)):
                ...
                flag_order_closed = self.OrderAdd_10(individual_ID_run_EA, order_id, new_closed_order, self.ORDER_CLOSED_DICT)    
...
#             print("Successfully Close the Opened order %s." % order_id)
            log.info("{0}: Successfully Close the Opened order {1}.".format(individual_ID_run_EA, order_id))
        else:
#             print("Error when closing the Opened order %s." % order_id)
            log.info("{0}: Error when closing the Opened order {1}.".format(individual_ID_run_EA, order_id))
 
        return flag_order_closed
          
    #===============================================================================
    def DeletePendingOrder19_3(self, individual_ID_run_EA):
        ...
        for order_id in self.ORDER_OPENED_DICT.keys():
            ...
            if (order_type == OP_SELLLIMIT):
                ...
                if (self.OrderDelete_4(individual_ID_run_EA, order_id, self.ORDER_OPENED_DICT)):
                    ...
                    self.OrderAdd_10(individual_ID_run_EA, order_id, order, self.ORDER_DELETED_DICT)
         
    #===============================================================================
    def DeletePendingOrder18_3(self, individual_ID_run_EA):
    ...
    #===============================================================================
    def CheckLastOrderType33_4(self, individual_ID_run_EA):
        ...
        if (orderType == OP_SELL or self.FirstTime33):
            self.FirstTime33 = False
            self.DeletePendingOrder19_3(individual_ID_run_EA)
    ...
    #===============================================================================
    def OrderSend_9(self, order_id, order_type, order_lots, order_price, order_slippage, order_sl,
                    order_tp, individual_ID_run_EA):
        ...
        * order_expire: Order expiration time (for pending orders only).'''
        ...
        if (order_type != OP_BUYLIMIT and order_type != OP_SELLLIMIT):
            log.info("{0}: This is not a Pending order OP_BUYLIMIT or OP_SELLLIMIT. Its type is {1}.".format(self.individual_I, order_type))
            return False
        # create a pending order OP_BUYLIMIT or OP_SELLLIMIT
            ...
            flag_added = self.OrderAdd_10(individual_ID_run_EA, order_id, order, self.ORDER_OPENED_DICT)
             
            if (flag_added):
                return True
             
    #===============================================================================
    def BuyPendingOrder13_8(self, individual_ID_run_EA):
        ''' Buy Pending Order OP_BUYLIMIT. '''
         
        # exit when reaching maximum orders per day
        if (self.MaxOrders_9(self.OPENORDERSLIMITDAY, individual_ID_run_EA)):
            return
         
        # calculate all variables for placing an order
        price = abs((self.NormalizeDouble_9(self.ask_price, self.NDigits) 
                 + (abs(self.ARRANGEMENTS_OF_TRADES) * self.PipValue) * self.my_point))
        ...
        ticket = self.OrderSend_9(order_id, OP_BUYLIMIT, self.Lots, price, self.SLIPPAGE, SL, TP, individual_ID_run_EA)
         
        # inform the result after placing the pending order OP_BUYLIMIT
        if (ticket == False):
#             print("OrderSend_9() error for OP_BUYLIMIT %s" % order_id)
            log.info("{0}: OrderSend_9() error for OP_BUYLIMIT {1}".format(individual_ID_run_EA, order_id))
        else:
#             print("Successfully placed the Pending order OP_BUYLIMIT %s." % order_id)
            log.info("{0}: Successfully placed the Pending order OP_BUYLIMIT {1}.".format(individual_ID_run_EA, order_id))
             
    #===============================================================================
    def SellPendingOrder14_8(self, individual_ID_run_EA):
        ...
        ticket = self.OrderSend_9(order_id, OP_SELLLIMIT, self.Lots, price, self.SLIPPAGE, SL, TP, individual_ID_run_EA)
         
        # inform the result after placing the pending order OP_SELLLIMIT
        if (ticket == False):
            log.info("{0}: OrderSend_9() error for OP_SELLLIMIT {1)" .format(individual_ID_run_EA, order_id))
        else:
            log.info("{0}: Successfully placed the Pending order OP_SELLLIMIT {1}.".format(individual_ID_run_EA, order_id))
             
    #===============================================================================
    def IfSellOrderDoesNotExist4_7(self, Mode, individual_ID_run_EA):
        ...
        for order_id in self.ORDER_OPENED_DICT.keys():
            ...
            if self.OrderType_5(order_id, self.ORDER_OPENED_DICT, individual_ID_run_EA) == OP_SELL:
        ...
        if (exists == False and Mode == DEFAULT_SECOND_NUMBER_INT):
            self.BuyPendingOrder13_8(individual_ID_run_EA)
         
        if (exists == True and Mode == 2):
            self.DeletePendingOrder18_3(individual_ID_run_EA);
     
    #===============================================================================
    def IfBuyOrderDoesNotExist6_7(self, Mode, individual_ID_run_EA):
        ...
        for order_id in self.ORDER_OPENED_DICT.keys():
            ...
            if self.OrderType_5(order_id, self.ORDER_OPENED_DICT, individual_ID_run_EA) == OP_BUY:
        ...
        if (exists == False and Mode == DEFAULT_SECOND_NUMBER_INT):
            self.SellPendingOrder14_8(individual_ID_run_EA)
         
        if (exists == True and Mode == 2):
            self.DeletePendingOrder19_3(individual_ID_run_EA);
     
    #===============================================================================
    def IfOrderDoesNotExist11_6(self, individual_ID_run_EA):
        ...
        for order_id in self.ORDER_OPENED_DICT.keys():
            ...
            # when the order is OP_BUYLIMIT
            if self.OrderType_5(order_id, self.ORDER_OPENED_DICT, individual_ID_run_EA) == OP_BUYLIMIT:
        ...
        if (exists == False):
            self.IfSellOrderDoesNotExist4_7(1, individual_ID_run_EA)
         
        if (exists == True):
            self.IfSellOrderDoesNotExist4_7(2, individual_ID_run_EA);
         
    #===============================================================================
    def IfOrderDoesNotExist12_6(self, individual_ID_run_EA):
        ...
        for order_id in self.ORDER_OPENED_DICT.keys():
            ...
            # when the order is OP_SELLLIMIT
            if self.OrderType_5(order_id, self.ORDER_OPENED_DICT, individual_ID_run_EA) == OP_SELLLIMIT:
        ...
        if (exists == False):
            self.IfBuyOrderDoesNotExist6_7(1, individual_ID_run_EA)
         
        if (exists == True):
            self.IfBuyOrderDoesNotExist6_7(2, individual_ID_run_EA);
    ...
    #===============================================================================
    def ATRFilter_5(self, individual_ID_run_EA):
        ''' ATRFilter_5. '''
        ...
        if ((((ATRPrePips1 >= self.ATRDnLimit1 and ATRPrePips1 <= self.ATRUpLimit1 and ATRPrePips2 >= self.ATRDnLimit1 and ATRPrePips2 <= self.ATRUpLimit1 and atr_p >= self.ATRDnLimit1 and atr_p <= self.ATRUpLimit1) 
            ...
            log.info("{0}: {1}".format(individual_ID_run_EA, ATR))
            self.IfOrderDoesNotExist11_6(individual_ID_run_EA)
            self.IfOrderDoesNotExist12_6(individual_ID_run_EA)
        ...
        # DEMO --> END here
            ...
            if((self.DeleteOrderATR == True) and (self.FILTERING == True)):
                self.CloseOrder16_3(individual_ID_run_EA)
                self.CloseOrder17_3(individual_ID_run_EA)
    ...
    #===============================================================================
    def LimitOpenOrders28_4(self, individual_ID_run_EA):
        ...
        if (len(self.ORDER_OPENED_DICT) == DEFAULT_NUMBER_INT or self.OPENORDERSLIMIT == DEFAULT_NUMBER_INT):
            self.ATRFilter_5(individual_ID_run_EA)
        else:
    ...
    #===============================================================================
    def CheckLastOrderType35_4(self, individual_ID_run_EA):
    ...
    #===============================================================================
    def HoursFilter22_3(self, individual_ID_run_EA):
        ''' HoursFilter22_3. '''
        ...
        if (self.NewsTime_4() == False 
            ...
            if (self.FILTERSPREAD and self.mode_spread > self.SpreadMax):
                log.info("{0}: Spread too high! Max spread: {1} - Current spread: {2}. Date: {3}".format(individual_ID_run_EA, self.SpreadMax, self.mode_spread, self.current_datetime))
                return
  
            self.CheckLastOrderType33_4(individual_ID_run_EA)
            self.LimitOpenOrders28_4(individual_ID_run_EA)
            self.CheckLastOrderType35_4(individual_ID_run_EA)
        else:
    ...
    #===============================================================================
    def WeekdayFilter23_2(self, individual_ID_run_EA):
        ''' WeekdayFilter23_2  '''
        ...
        if ((self.MONDAY and self.DayOfWeek_3() == 0) 
            ...
            self.Overide = False
            self.HoursFilter22_3(individual_ID_run_EA)
    ...
    #===============================================================================
    def CloseOrder17_3(self, individual_ID_run_EA):
        ...
        for order_id in self.ORDER_OPENED_DICT.keys():
            ...
            if  (order_type == OP_BUY):
                ...
                self.OrderClose_4(order, order_id, order_type, individual_ID_run_EA)
         
        self.DeletePendingOrder18_3(individual_ID_run_EA)         
         
    #===============================================================================
    def CloseOrder16_3(self, individual_ID_run_EA):
    ...
    #===============================================================================
    def CloseOrderIf21_2(self, individual_ID_run_EA):
    ...
    #===============================================================================
    def AtCertainTime6_2(self, individual_ID_run_EA):
        ''' Return  '''
    ...
    #===============================================================================
    def OnEveryTick24_1(self, individual_ID_run_EA):
        ''' Actions for every tick  '''
         
        # functions for placing Pending orders
        self.WeekdayFilter23_2(individual_ID_run_EA)
        self.AtCertainTime6_2(individual_ID_run_EA)
         
        # TODO: Extra Functions for having Open/Market orders: 
        self.ModifyPendingOrder_2(individual_ID_run_EA)
         
        # function for closing orders
        self.CloseOrderIf21_2(individual_ID_run_EA)
                 
    #===============================================================================
    def CloseDeleteAll_1(self, individual_ID_run_EA):
        ''' Close all orders  '''
        ...
        for order_id in self.ORDER_OPENED_DICT.keys():
            ...
            if  (order_type == OP_BUY or order_type == OP_SELL):
                # --> Close a specific old_opended_order in data by deleting this OP_BUY in the Opened and Pending orders pool 
                # and return new value of order from Opened position to Close position
                flag_close_delete_all = self.OrderClose_4(order, order_id, order_type, individual_ID_run_EA)
            ...
            if  (order_type == OP_BUYLIMIT or order_type == OP_SELLLIMIT):
                ...
                flag_close_delete_all = self.OrderDelete_4(individual_ID_run_EA, order_id, self.ORDER_OPENED_DICT)
                ...
                if (flag_close_delete_all):
                    ...
                    order[DAY_COL_INDEX] = self.current_day
                    order[TIME_COL_INDEX] = self.current_time
                    flag_close_delete_all = self.OrderAdd_10(individual_ID_run_EA, order_id, order, self.ORDER_DELETED_DICT)
    ...
    #===============================================================================
    def CheckEnoughMoney_2(self, order, individual_ID_run_EA):
        ...
        '''  margin = order[PRICE_ENTRY_COL_INDEX] * ONE_LOT_VALUE / LEVERAGE '''
        ...
        if (free_magin >= margin):
            ...
            log.info("{0}: There are not enough money in account to open order {1}".format(individual_ID_run_EA, order[ORDER_ID_COL_INDEX]))
            return False
         
    #===============================================================================
    def ModifyPendingOrder_2(self, individual_ID_run_EA):
        ...
        if (self.ords_in_a_day >= self.OPENORDERSLIMITDAY):
            log.info("{0}: Cannot modify this Pending order due to reaching maximum {1} orders per day.".format(individual_ID_run_EA, self.OPENORDERSLIMITDAY))
        else:
            # get all Opened and Pending orders in the pool
            # ORDER_OPENED_DICT = {order_id: ['Date_Time', 'Day', 'Time', 'Type', 'OrderID', 'Size', 'EntryPrice', 'ExitPrice', 'SL', 'TP', 'Profit', 'Balance']}
            for order_id in self.ORDER_OPENED_DICT.keys():
                ...
                if  (order_type == OP_BUYLIMIT or order_type == OP_SELLLIMIT):
                    ...
                    if (order_type == OP_BUYLIMIT):
                        ...
                        if (entry_price <= exit_price
                            ...
                            flag_modified = False
                            log.info("{0}: Cannot modify order OP_BUYLIMIT {1} due to entry_price <= exit_price.".format(individual_ID_run_EA, order_id))
                    ...
                    # compare entry price with exit price from next tick
                        ...
                        if (entry_price >= exit_price
                            ...
                            flag_modified = False
                            log.info("{0}: Cannot modify order OP_SELLLIMIT {1} due to entry_price >= exit_price.".format(individual_ID_run_EA, order_id))
                             
                    # when this Pending order can be modified
                    if (flag_modified):
                        # modified this Pending order to become Open order when having enough money
                        if (self.CheckEnoughMoney_2(order, individual_ID_run_EA)):  
                            # delete this order in the Opened and Pending orders pool
                            if (self.OrderDelete_4(individual_ID_run_EA, order_id, self.ORDER_OPENED_DICT)):
                                ...
                                if (self.OrderAdd_10(individual_ID_run_EA, order_id, order, self.ORDER_OPENED_DICT)):
                                    self.ords_in_a_day += DEFAULT_SECOND_NUMBER_INT
                                    log.info("{0}: {1}".format(individual_ID_run_EA, message))
                        else:
                            log.info("{0}: There are NOT enough money to modify this Pending order {1} to Open order.".format(individual_ID_run_EA, order_id))
    ...
    #===============================================================================
    def analised_tick_data(self, individual_ID_run_EA, PARAMETERS_DATA, folder_output_individual, file_):
        ...
        time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
        log.info("==> {0}: Loading TICK DATA from file: {1}...".format(individual_ID_run_EA, file_basename))
        print("{0} ==> {1}: Loading TICK DATA from file: {2}...".format(time_stamp, individual_ID_run_EA, file_basename))
        '''
        ...
        while (current_tick < len(TICK_DATA) - DEFAULT_SECOND_NUMBER_INT):
            ...
            if (current_tick == DEFAULT_NUMBER_INT):
                log.info("... ==> {0}: start processing the data...".format(individual_ID_run_EA))
                print("{0}... ==> {1}: start processing the data...".format(time_stamp, individual_ID_run_EA))
            '''
            ...
            if (fprevious_day != self.current_day):
                ...
                ''' For EA running by itself 
                time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
                log.info("==> {0}: current_tick: {1}".format(individual_ID_run_EA, current_tick))
                log.info("==> {0}: checking date {1}".format(individual_ID_run_EA, self.current_datetime))
                print("{0} ==> {1}: current_tick: {2}".format(time_stamp, individual_ID_run_EA, current_tick))
                print("{0} ==> {1}: checking date {2}".format(time_stamp, individual_ID_run_EA, self.current_datetime))
                  
                run_percentage = round((float(current_tick) / float(len(TICK_DATA))) * float(100), 2)
                  
                log.info("... ==> {0}: processing {1}% of the data, date {2}...".format(individual_ID_run_EA, run_percentage, self.current_datetime))
                time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
                print("{0}... ==> {1} processing {2}% of the data, date {3}...".format(time_stamp, individual_ID_run_EA, run_percentage, self.current_datetime))
                '''
            ...
            if (self.MaxOrders_9(self.OPENORDERSLIMITDAY, individual_ID_run_EA)):
                ...
                if (self.clear):                       
                    if (self.CloseDeleteAll_1(individual_ID_run_EA)):
                        self.clear = False
                    else:
                        return
                ...
                if (self.SAFEEQUITYSTOPOUT 
                    ...
                    and self.CloseDeleteAll_1(individual_ID_run_EA) == False):
                            self.clear = True
                            return 
                ...
                self.OnEveryTick24_1(individual_ID_run_EA)
         
        ''' For EA running by itself 
        time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
        log.info("==> {0}: Completed running tick data file: {1} with profit {2}!".format(individual_ID_run_EA, file_basename, self.net_profit))
        print("{0} ==> {1} Completed running tick data file: {2} with profit {3}!".format(time_stamp, individual_ID_run_EA, file_basename, self.net_profit))
        '''
                               
        # Write out other data for reference
        file_basename = file_basename.split('.')[DEFAULT_NUMBER_INT]
        write_value_of_dict2csv_no_header(self.ORDER_CLOSED_DICT, folder_output_individual + file_basename + '_' + FILENAME_ORDER_CLOSED_HISTORY)
        write_value_of_dict2csv_no_header(self.ORDER_DELETED_DICT, folder_output_individual + file_basename + '_' + FILENAME_ORDER_DELETED_HISTORY)
    ...
    #===============================================================================
    def ea_run(self, PARAMETERS_DATA, individual_ID_run_EA, RUN_FREQUENCY_BY_YEAR,
               flag_outputfile, folder_optimization_output):
        ''' EA running '''
         
        time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
        log.info("==> Start calculating fitness for individual {0} for {1} year(s)!".format(individual_ID_run_EA, RUN_FREQUENCY_BY_YEAR))
        print("{0} ==> Start calculating fitness for individual {1} for {2} year(s)!".format(time_stamp, individual_ID_run_EA, RUN_FREQUENCY_BY_YEAR))
             
        total_profit = DEFAULT_NUMBER_FLOAT 
         
        # Create an folder for storing all outputs in this section
        symbol_folder_output = folder_optimization_output + 'backtests_output'
        if os.path.exists(symbol_folder_output) == False:
            os.makedirs(symbol_folder_output)
         
        folder_output_individual = symbol_folder_output + '/ind_' + str(individual_ID_run_EA) + '_outputs/'
        if os.path.exists(folder_output_individual) == False:
            os.makedirs(folder_output_individual)
        ...
        ''' SINGLETHREADING 
        for file_ in allFiles:
            total_profit += HappyForexEA().analised_tick_data(individual_ID_run_EA, PARAMETERS_DATA, folder_output_individual, file_,)
        '''
        ...
        ''' For EA running by itself 
        time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
        log.info("==> {0}: Pool size of ThreadPool: {1}!".format(individual_ID_run_EA, pool_size))
        print("{0} ==> {1}: Pool size of ThreadPool: {2}!".format(time_stamp, individual_ID_run_EA, pool_size))
        '''
        ...
        # Running EA process
        results = [ea_pool.apply_async(HappyForexEA().analised_tick_data, (individual_ID_run_EA, PARAMETERS_DATA, folder_output_individual, file_,))
        ...
        # write out other data for reference
        if (flag_outputfile):
            file_name_order_history = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + '.' + FILENAME_ORDER_CLOSED_HISTORY
            combine_all_files_in_a_folder(folder_output_individual, file_name_order_history,
                                          '*_' + FILENAME_ORDER_CLOSED_HISTORY)
             
            # convert back the Date Time for output file
            converted_data = convert_datetime_back_whole_list(file_name_order_history)
             
            # create a new file name
            write_array2csv_with_delimiter_no_header(converted_data, file_name_order_history, ',')
             
        time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
        log.info("==> Completed calculating fitness for individual {0} with profit {1}!".format(individual_ID_run_EA, round(total_profit, 2)))
        print("{0} ==> Completed calculating fitness for individual {1} with profit {2}!".format(time_stamp, individual_ID_run_EA, round(total_profit, 2)))
...
class Individual(object):
    ...
    def create_a_set_of_genes(self):
        ...
        random_num = random.randint(DEFAULT_SECOND_NUMBER_INT, abs(int(self.genes[row][col])))
        self.genes[row][col] = random_num * (-1)  
        row += DEFAULT_SECOND_NUMBER_INT
        # --> Lots ==> pickup value 0.01 to 0.1
        default_lot = 0.01
        random_num = default_lot + (default_lot * random.randint(DEFAULT_SECOND_NUMBER_INT, 9))
...
#             self.all_genes_pool[an_individual.genes_completed] = i
    ...
    def cal_fitness(self, ind_, RUN_FREQUENCY_BY_YEAR, flag_outputfile, folder_optimization_output):
         
        ind_.fitness = DEFAULT_NUMBER_INT
         
        # run the EA logic to return the profit
        ind_.net_profit = self.happyforex_EA_instance.ea_run(ind_.genes_completed,
                                                             ind_.individual_ID,
                                                             RUN_FREQUENCY_BY_YEAR,
                                                             flag_outputfile,
                                                             folder_optimization_output)
#         ind_.net_profit = self.happyforex_EA_instance.ea_run_testing()
        ...
        self.FITNESS_DICT[str(ind_.individual_ID) + '_origin'] = '$' + str(round(ind_.net_profit, 2))
    ...
    #===============================================================================
    # Calculate the fitness of each individual
    def calculate_fittest(self, RUN_FREQUENCY_BY_YEAR, flag_outputfile, folder_optimization_output):
...
#         individuals_w_fitness = ea_pool.map(self.cal_fitness, self.individuals)
        results = [ea_pool.apply_async(self.cal_fitness, (ind_, RUN_FREQUENCY_BY_YEAR, flag_outputfile, folder_optimization_output,)) for ind_ in self.individuals]
...
class HappyForexGenericAlgorithm(object):
    ...
    def add_fittest_offspring(self, RUN_FREQUENCY_BY_YEAR, folder_optimization_output):
        ...
        results = [ea_pool.apply_async(self.population.cal_fitness,
                                       (ind_, RUN_FREQUENCY_BY_YEAR, flag_outputfile, folder_optimization_output,)) for ind_ in highest_individuals]
...
##########################           MAIN           ############################
################################################################################     
def ga_run(RUN_FREQUENCY_BY_YEAR, folder_optimization_output):
    ...
    print('#============================== Initialize population_%s ==============================' % time_stamp)
    ...
    print('#============================== Write the individual_ID_list to a CSV file_%s ==============================' % time_stamp)
    write_wholedict2csv_no_header(happyforexGA.population.individuals_ID_dict, folder_optimization_output + FILENAME_POPULATION_INITIAL)
      
    # Calculate fitness of each individual
    flag_outputfile = False
    time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
    log.info('#============================== Calculate fitness of each individual ==============================')
    print('#============================== Calculate fitness of each individual_%s ==============================' % time_stamp)
    happyforexGA.population.calculate_fittest(RUN_FREQUENCY_BY_YEAR, flag_outputfile, folder_optimization_output)
    ...
    print('#============================== Get the individual with highest fitness_%s ==============================' % time_stamp)
    happyforexGA.fittest_ind = happyforexGA.population.get_fittest()
            
    time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
    log.info('#============================== Population gets an individual with maximum fitness ==============================')
    print('#============================== Population gets an individual with maximum fitness_%s ==============================' % time_stamp)
    ...
    while (happyforexGA.population.fittest < MAX_FITNESS 
            and happyforexGA.generationCount < happyforexGA.population.popSize):  # TODO: for testing only
         
        # count generation
        happyforexGA.generationCount += DEFAULT_SECOND_NUMBER_INT
         
        time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
        log.info('#============================== GENERATION_' + str(happyforexGA.generationCount) + ' ==============================')
        print('#============================== GENERATION_%s ==============================' % happyforexGA.generationCount)
            
        # Do selection
        log.info('#============================== population selection ==============================')
        print('#============================== population selection_%s ==============================' % time_stamp)
        ...
        print('#============================== population crossover_%s ==============================' % time_stamp)
        ...
        if random.randint(DEFAULT_NUMBER_INT, MAX_FITNESS) < MAX_FITNESS:
            ...
            print('#============================== population mutation_%s  ==============================' % time_stamp)
        ...
        time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
        happyforexGA.add_fittest_offspring(RUN_FREQUENCY_BY_YEAR, folder_optimization_output)
        ...
        # Print out and write to CSV file the temporary highest solution
        time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
        ...
        if happyforexGA.fittest_ind.fitness < MAX_FITNESS:
            file_path_highest_solution = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + FILENAME_BEST_FITNESS
            file_path_highest_parameters = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + FILENAME_BEST_PARAMETERS
        ...
        write_wholedict2csv_no_header(happyforexGA.population.FITNESS_DICT, folder_optimization_output + 'ProfitList.csv')
         
        # Print out Order history of the temporary highest solution
        folder_output = symbol_folder_output + '/ind_' + happyforexGA.fittest_ind.individual_ID + '_outputs/'
        file_name_order_history = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + FILENAME_BEST_FITNESS_ORDER_HISTORY
        combine_all_files_in_a_folder(folder_output, file_name_order_history,
                                      '*_' + FILENAME_ORDER_CLOSED_HISTORY)
    ...
    file_path_best_solution = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + FILENAME_BEST_FITNESS
    file_path_best_parameters = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + FILENAME_BEST_PARAMETERS
     
    write_array2csv_with_delimiter_no_header(happyforexGA.fittest_ind.genes, file_path_best_solution, '=')
    write_array2csv_with_delimiter_no_header(happyforexGA.fittest_ind.genes_completed, file_path_best_parameters, '=')
    ...
    print('#============================== Write the population final to a CSV file_%s ==============================' % time_stamp)
    write_wholedict2csv_no_header(happyforexGA.population.individuals_ID_dict,
                             folder_optimization_output + FILENAME_POPULATION_FINAL.replace(".csv", "_" + str(happyforexGA.generationCount) + "th_gen.csv"))
     
    # Delete the symbol_folder_output to free memory
...
if __name__ == "__main__":
    ...
    # Create an folder for storing all outputs in this section 
    folder_optimization_output = convert_backflash2forwardflash(FOLDER_DATA_OUTPUT + 'Synergista_v1.00/30/' 
                                                                + str(int(RUN_FREQUENCY_BY_YEAR) * DAYS_OF_AYEAR) + '/' 
                                                                + SYMBOL + '/')
    if os.path.exists(folder_optimization_output):
        shutil.rmtree(folder_optimization_output)
    os.makedirs(folder_optimization_output)
      
    # If applicable, delete the existing log file to generate a fresh log file during each execution
    file_name_year_log = (folder_optimization_output 
                             + SYMBOL + '_' + YEAR_OUTPUT + '_' + FILENAME_LOG_GA)
      
    file_name_year_profile = (folder_optimization_output 
                             + SYMBOL + '_' + YEAR_OUTPUT + '_' + FILENAME_PROFILE_GA)
  
    if path.isfile(file_name_year_log):
        remove(file_name_year_log)
    if path.isfile(file_name_year_profile):
        remove(file_name_year_profile)
     
    # create a log file with properties     
    handler = logging.handlers.WatchedFileHandler(os.environ.get("LOGFILE", file_name_year_log))
    ...
    try:
        ...
        cProfile.run('ga_run(' + str(RUN_FREQUENCY_BY_YEAR) + ',\''  
                        + str(folder_optimization_output) + '\')'
                     , file_name_year_profile)
...
#         exit(1) 
    ...
    folder_optimization_output = convert_backflash2forwardflash(FOLDER_DATA_OUTPUT + 'Synergista_v1.00/30/' 
                                                                + str(int(RUN_FREQUENCY_BY_YEAR) * DAYS_OF_AYEAR) + '/' 
                                                                + SYMBOL + '/')
    if os.path.exists(folder_optimization_output) == False:
#         shutil.rmtree(folder_optimization_output)
        os.makedirs(folder_optimization_output)
      
    # If applicable, delete the existing log file to generate a fresh log file during each execution
    file_name_year_log = (folder_optimization_output 
                             + SYMBOL + '_' + YEAR_OUTPUT + '_' + FILENAME_LOG_BACKTEST)
       
    file_name_year_profile = (folder_optimization_output 
                             + SYMBOL + '_' + YEAR_OUTPUT + '_' + FILENAME_PROFILE_BACKTEST)
    ...
    try:
        ...
        # running EA
        cProfile.run('happyforex_EA_instance.ea_run(DEFAULT_PARAMETERS_DATA,' 
                     + individual_ID + ',' 
