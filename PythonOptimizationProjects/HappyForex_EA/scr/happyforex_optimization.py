...
from temp import convert_backflash2forwardflash
...
FILENAME_HIGHEST_FITNESS = '.30.solution_optimize_temporary.set'
FILENAME_BEST_FITNESS = '.30.solution_optimize_last.set'
FILENAME_HIGHEST_FITNESS_ORDER_HISTORY = '.30.order_history_optimize_temporary.set'
FILENAME_BEST_FITNESS_ORDER_HISTORY = '.30.order_history_optimize_last.set'
FILENAME_HIGHEST_PARAMETERS = '.30.optimize_temporary.set'
FILENAME_BEST_PARAMETERS = '.30.optimize_last.set'
...
if (len(sys.argv[1:]) == DEFAULT_NUMBER_INT):
    ...
    if (str(RUN_FREQUENCY_BY_YEAR) == RUN_FREQUENCY_1_YEAR
        ...
        or str(RUN_FREQUENCY_BY_YEAR) == RUN_FREQUENCY_11_YEAR):
    
        YEAR_OUTPUT = str(RUN_FREQUENCY_BY_YEAR) + 'years'
    else:
        RUN_FREQUENCY_BY_YEAR = DEFAULT_SECOND_NUMBER_INT
        YEAR_OUTPUT = '1year'
...
# FOLDER_TICK_DATA_ORIGINAL = FOLDER_TICK_DATA_ORIGINAL + '/Col3/'
...
#         ''' For EA running by itself 
#         time_stamp = datetime.now().strftime(TIME_STAMP_FORMAT)
#         log.info("==> {0}: Pool size of ThreadPool: {1}!".format(individual_ID, pool_size))
#         print("{0} ==> {1}: Pool size of ThreadPool: {2}!".format(time_stamp, individual_ID, pool_size))
#         ''' 
#         
...
#             file_path_highest_solution = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + FILENAME_HIGHEST_FITNESS
#             file_path_highest_parameters = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + FILENAME_HIGHEST_PARAMETERS
#         
...
#         file_name_order_history = folder_optimization_output + SYMBOL + '.' + str(RUN_FREQUENCY_BY_YEAR * DAYS_OF_AYEAR) + FILENAME_HIGHEST_FITNESS_ORDER_HISTORY
...
# 
# #     # Create a socket to receive stop command
# #     try:
# #         serverSocket = ServerSocket('conf/app.properties')
# #         start_new_thread(serverSocket.startReceiveData, ())
# #     except Exception as e:
# #         print('error: %s' % (str(e)))
# #         log.info('error: %s' % (str(e)))
# #         return
# #     
# #     def stop_by_specified_time():
# #         
# #         global OPT_DAYS_BEFORE_STOP
# #         global CURRENT_TASK_START_TIME
# #         global STOP_FLAG
# #         
# #         while True:
# #             # If OPT_DAYS_BEFORE_STOP is specified and optimization is still running
# #             if OPT_DAYS_BEFORE_STOP > 0 and STOP_FLAG == False:
# #                 # If it's time to stop
# #                 if datetime.now() - CURRENT_TASK_START_TIME >= timedelta(days=OPT_DAYS_BEFORE_STOP):
# #                     STOP_FLAG = True
# #                     print('Optimization is being stopped by specified time in configuration')
# #                     log.info('Optimization is being stopped by specified time in configuration')
# #             
# #             time.sleep(0.5)
# #         
# #         return
# #     
# #     start_new_thread(stop_by_specified_time, ())
# 
#     #===============================================================================
#     ''' RUNNING WHOLE EA WITH GENETIC OPTIMIZATION WITH LOG AND PROFILE
#     # Create an folder for storing all outputs in this section 
#     folder_optimization_output = convert_backflash2forwardflash(FOLDER_DATA_OUTPUT + '30/' 
