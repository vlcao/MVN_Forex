...
# Resample to 1 Hour (this format is needed) as per ohlc_dict, then remove any line with a NaN
df = df.resample('H').agg(ohlc_dict).dropna(how='any') 
...
# Write out to CSV
M1HFilePath = scrDirName + 'AUDUSD1_9ys_H1.csv'
df.to_csv(M1HFilePath)
